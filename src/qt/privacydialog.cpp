...
#include "zoxygencontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zOXYGEN ought to be enough for anybody." - Bill Gates, 2017
    ui->zOXYGENpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzOXYGENSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzOXYGEN->setEnabled(false);
        ui->pushButtonMintzOXYGEN->setToolTip(tr("zOXYGEN is currently disabled due to maintenance."));

        ui->pushButtonSpendzOXYGEN->setEnabled(false);
        ui->pushButtonSpendzOXYGEN->setToolTip(tr("zOXYGEN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOXYGENpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzOXYGEN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOXYGEN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOXYGEN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOXYGEN in ") +
...
void PrivacyDialog::on_pushButtonSpendzOXYGEN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOXYGEN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zOXYGEN
        sendzOXYGEN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOXYGEN
    sendzOXYGEN();
}

void PrivacyDialog::on_pushButtonZOXYGENControl_clicked()
{
    ZOXYGENControlDialog* zOXYGENControl = new ZOXYGENControlDialog(this);
    zOXYGENControl->setModel(walletModel);
    zOXYGENControl->exec();
}

void PrivacyDialog::setZOXYGENControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOXYGENSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOXYGEN()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid OXYGEN Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zOXYGENpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOXYGEN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " OXYGEN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOXYGEN</b>";
    ...
    // use mints from zOXYGEN selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZOXYGENControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZOXYGENControlDialog::GetSelectedMints();
    }
    ...
    // Clear zoxygen selector in case it was used
    ZOXYGENControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOXYGEN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " OXYGEN, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zOXYGEN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOXYGEN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOXYGEN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOXYGEN "));
    ui->labelzOXYGENAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzOXYGENSyncStatus->setVisible(fShow);
}
