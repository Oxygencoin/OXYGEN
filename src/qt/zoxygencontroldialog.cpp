...
#include "ui_zoxygencontroldialog.h"
...
std::list<std::string> ZOXYGENControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZOXYGENControlDialog::listMints;

ZOXYGENControlDialog::ZOXYGENControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZOXYGENControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZOXYGENControlDialog::~ZOXYGENControlDialog()
{
    delete ui;
}

void ZOXYGENControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOXYGENControlDialog::updateList()
{
...
void ZOXYGENControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOXYGENControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZOXYGEN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOXYGENControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZOXYGENControlDialog::GetSelectedMints()
{
...
void ZOXYGENControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
