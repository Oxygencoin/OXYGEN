...
# JSON produced by oxygend, do NOT use it to try to
...
function CreateTxn1 {
  ...
  RAWTOXYGEN=$( $CLI $1 createrawtransaction "[{\"txid\":\"$TXID\",\"vout\":$VOUT}]" "{\"$3\":$AMOUNT}")
  ExtractKey hex "$( $CLI $1 signrawtransaction $RAWTOXYGEN )"
}
